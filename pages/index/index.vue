<template>
	<view class="nav_bar_custom">
		...
		<!-- 文本 -->
		<view class="Sent_information backdrop your_element"
		...
		<view v-if="messageData.length <= 0" class="Sent_information backdrop widthAuto your_element">
			<view class="nav_content">
				...
				<view class="Default_problem" v-for="(item,index) in problemDate" 
				:key="index" @click="selectText(item)">{{item}}</view>
			</view>
		</view>
		<!-- 问答区域 -->
		<!-- 用户提问 -->
		<block v-for="(item,index) in messageData ":key="index" >
			...
			<view class="Sent_information backdrop" v-else>
				...
				<view class="ai_content" v-if="item.content != ''">
					...
					<image v-if="item.copyIcon" @click="copyData(item.content)" class="img" src="../../static/复制.png" mode="heightFix"></image>
				</view>
			</view>
		</block>
		<!-- 底部，输入框 -->
		<view class="Input_field">
			<view><image class="img" @click="clearMessage" src="../../static/刷新.png" mode="aspectFill"></image></view>
			<input class="input"
			...
			>
			<view><image class="img" src="../../static/发送.png" mode="aspectFill" @click="sendMessage"></image></view>
		</view>
